...
export function demoReply(text:string,notes:string[]):string {
 const match=text.match(/^(?:(?:what is|calculate|solve)\s+)?(-?\d+(?:\.\d+)?)\s*([+×*÷/−-])\s*(-?\d+(?:\.\d+)?)[?.!]?(?:\s+explain[^0-9]*)?$/i);
 if(match){const a=Number(match[1]),b=Number(match[3]),op=match[2];if((op==='/'||op==='÷')&&b===0)return 'Division by zero is undefined. There is no number that, multiplied by zero, gives a nonzero dividend.';const n=op==='+'?a+b:op==='-'||op==='−'?a-b:op==='*'||op==='×'?a*b:a/b;if(!Number.isFinite(n))return 'That result is outside the demo calculator’s supported range. Use an AI provider for this question.';return `${a} ${op} ${b} = ${Number(n.toPrecision(12))}.${(op==='×'||op==='*')&&b>=10&&Number.isInteger(b)?` One way to work it out: ${a} × ${Math.floor(b/10)*10} + ${a} × ${b%10} = ${Number(n.toPrecision(12))}.`:''}\n\nNotice the parietal region: quantity processing often works alongside frontal reasoning and language networks.`}
