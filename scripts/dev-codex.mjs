...
const login=spawnSync(codex,['login','status'],{stdio:'inherit'});
if(login.error||login.status!==0)throw new Error('Run codex login before starting Mindlight.');
...
start('scripts/codex-bridge.mjs',[],{CODEX_BIN:codex,CODEX_BRIDGE_TOKEN:token});
let ready=false;
...
if(!stopping){if(ready)start('scripts/run-framework.mjs',['dev']);else{console.error('The local Codex bridge could not start.');stop(1)}}
