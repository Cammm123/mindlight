...
const server=http.createServer(async(req,res)=>{
 ...
 let messages;try{messages=JSON.parse(raw).messages;if(!Array.isArray(messages)||messages.length>21||messages.some(m=>!['system','user','assistant'].includes(m.role)||typeof m.content!=='string'||m.content.length>16000))throw new Error()}catch{return json(400,{error:'Invalid messages'})}
